...
const FlexContent = ({ endpoint, isHighlight = false }) => {

    const { heading, title, text, img, url, btn } = endpoint || {};

    return (
        <div className={`flex items-center justify-between lg:flex-col lg:justify-center nike-container ${ isHighlight ? 'flex-row-reverse' : 'flex-row' }`}>
            ...
            <div className='flex items-center justify-center max-w-xl relative lg:max-w-none w-full'>
                <img src={img} className={`w-auto object-fill transitions-theme ${ isHighlight ? 'h-60 lg:h-56 md:h-52 sm:h-44 xs:h-36 rotate-6 hover:-rotate-12' : 'h-72 lg:64 md:h-60 sm:h-48 xs:h-40 rotate-[19deg] hover:rotate-12' }`} alt={`img/${heading}`} />
            </div>
        </div>
    )
}
